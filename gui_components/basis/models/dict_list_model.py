...
class DictListModel(QAbstractItemModel):
    def __init__(self, headers, data, parent=None):
        super(DictListModel, self).__init__(parent)
        ...
        if type(data) == dict:
            ...
            for dict_keys, dict_values in data.items():
                if isinstance(dict_values, list):
                    # if dict_values contains a dict
                    for list_values in dict_values:
                        if isinstance(list_values, dict):
                            ...
                            parent.child(parent.childCount() - 1).setData(0, list_values["type"])
                            parent.child(parent.childCount() - 1).setData(1, list_values.get("id", "None"))
                            self.createData(list_values, indent)
